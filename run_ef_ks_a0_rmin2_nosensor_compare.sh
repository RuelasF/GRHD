...
#
# Both runs use the same logarithmic grid, r_min=2, no shock sensor,
# WENO5/HLLE, and T=1000. The base initialization file is restored exactly
# on every exit.
...
campaign_log="${CAMPAIGN_LOG:-log_FM_EFKS_a0_rmin2_nosensor_compare_v1_campaign.txt}"
...
for tag in "${metric_tags[@]}"; do
  artifact_id="FM_EFKS_a0_rmin2_nosensor_v1_${tag}"
...
for index in "${!metric_tags[@]}"; do
  ...
  sed -i -E \
    ...
    -e "/subroutine setup_fishbone_moncrief_equatorial\(\)/,/end subroutine setup_fishbone_moncrief_equatorial/ s/^[[:space:]]*use_shock_sensor[[:space:]]*=.*/    use_shock_sensor = .false./" \
  ...
  [[ "$(extract_fm_assignment use_shock_sensor)" == "    use_shock_sensor = .false." ]]
...
echo "EF/KS a=0 r_min=2 no-shock-sensor comparison completed."
