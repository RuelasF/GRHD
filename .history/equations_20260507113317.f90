...
contains
  ...
  subroutine prim_to_cons(prim_state, cons_state, i, j, k)
    ...
    ! Cuadrado de la velocidad Euleriana tridimensional (Métrica diagonal pura)
    ...
    ! Momentos espaciales covariantes S_i usando métrica espacial g_ii diagonal
    cons_state(eq_vx) = prim_state(eq_de) * h * W**2 * v1 * g(1)
    cons_state(eq_vy) = prim_state(eq_de) * h * W**2 * v2 * g(2)
    cons_state(eq_vz) = prim_state(eq_de) * h * W**2 * v3 * g(3)
  ...
  subroutine cons_to_prim(cons_state, prim_state, i, j, k)
    ...
    ! 2. DINAMIZAR EL SUELO DE LA ATMÓSFERA
    ...
    S_sq  = cons_state(eq_vx)**2 / g(1) + cons_state(eq_vy)**2 / g(2) + cons_state(eq_vz)**2 / g(3)
    ...
    ! --- ESCUDO A: ERROR DE SOLUCIONADOR ---
    if (W < 1.0d0) then
      prim_state(eq_de) = rho_floor
      prim_state(eq_pr) = p_floor
      prim_state(eq_vx:eq_vz) = 0.0d0
    ...
    ! Inversión exitosa. Extraemos V y des-escalamos la métrica.
    ...
    prim_state(eq_vx) = cons_state(eq_vx) / (V * g(1))
    prim_state(eq_vy) = cons_state(eq_vy) / (V * g(2))
    prim_state(eq_vz) = cons_state(eq_vz) / (V * g(3))

    ! --- ESCUDOS POST-INVERSIÓN Y SANEAMIENTO ---
    if (prim_state(eq_de) < rho_floor) prim_state(eq_de) = rho_floor
    if (prim_state(eq_pr) < p_floor)   prim_state(eq_pr) = p_floor

    ! Eliminar o comentar esto para permitir 2.5D (Axisimetría con rotación)
    ! if (ny == 1) then
    !   prim_state(eq_vy) = 0.0d0
    !   cons_state(eq_vy) = 0.0d0
    ! end if
    ! 
    ! if (nz == 1) then
    !   prim_state(eq_vz) = 0.0d0
    !   cons_state(eq_vz) = 0.0d0
    ! end if
  ...
  subroutine eval_residual(W, cons_state, i, j, k, residual, d_residual)
    ...
    real*8 :: V, dV, Q_sq, g(3)

    ! Se lee la métrica localmente desde la caché en lugar de calcularla
    g(:) = gamma_c(:,i,j,k)
    call eval_V_and_dV(W, cons_state, V, dV)

    Q_sq = cons_state(eq_vx)**2/g(1) + cons_state(eq_vy)**2/g(2) + cons_state(eq_vz)**2/g(3)
  ...
  function root_safe_newton(cons_state, i, j, k, x1_in, x2_in) result(root)
    ...
    real*8 :: S_sq_local, W_max_guess, g_local(3)
    ...
    if (present(x2_in)) then 
      ...
      g_local(:) = gamma_c(:,i,j,k)
      S_sq_local = cons_state(eq_vx)**2/g_local(1) + cons_state(eq_vy)**2/g_local(2) + cons_state(eq_vz)**2/g_local(3)
    ...
    ! Llamadas actualizadas con i, j, k
    call eval_residual(x1, cons_state, i, j, k, f_low, df)
    call eval_residual(x2, cons_state, i, j, k, f_high, df)

    ! Falla de "Bracketing": Si los signos son iguales, se dispara fallback de Atmósfera
    ...
    ! Bucle iterativo de Newton-Raphson guiado
  ...
  subroutine calc_fluxes(prim_state, f_out, scan_dir, alpha, beta, g, sqg)
    ...
    real*8, intent(in)  :: alpha, beta(3), g(3), sqg
    ...
    v1 = prim_state(eq_vx); v2 = prim_state(eq_vy); v3 = prim_state(eq_vz)

    ! Cuadrado de la velocidad con la métrica espacial diagonal
    v_sq = v1**2 * g(1) + v2**2 * g(2) + v3**2 * g(3)
    ...
    if (scan_dir == DIR_X) then
      v_norm = v1; beta_norm = beta(1)
    else if (scan_dir == DIR_Y) then
      v_norm = v2; beta_norm = beta(2)
    else
      v_norm = v3; beta_norm = beta(3)
    end if
    ...
    ! Ecuaciones de Momento: F(S_i)
    f_out(eq_vx) = prim_state(eq_de) * h * W**2 * v1 * g(1) * v_eff + merge(alpha*prim_state(eq_pr), 0.0d0, scan_dir == DIR_X)
    f_out(eq_vy) = prim_state(eq_de) * h * W**2 * v2 * g(2) * v_eff + merge(alpha*prim_state(eq_pr), 0.0d0, scan_dir == DIR_Y)
    f_out(eq_vz) = prim_state(eq_de) * h * W**2 * v3 * g(3) * v_eff + merge(alpha*prim_state(eq_pr), 0.0d0, scan_dir == DIR_Z)
