...
contains
  ...
  subroutine setup_fishbone_moncrief_initial(prim_state, r_in, r_max_dens, K_poly)
    ...
    real*8  :: g_tt, rho_max_local

    local_nx = size(prim_state, 2)
    local_ny = size(prim_state, 3)
    local_nz = size(prim_state, 4)

    ! La métrica analítica SIEMPRE requiere el radio físico
    call calculate_metric(r_in, pi/2.0d0, alpha=alpha, beta=beta, gamma=g)
    ...
    do k = 1, local_nz
      do j = 1, local_ny
        do i = 1, local_nx
          ...
          ! Aseguramos métrica en el plano ecuatorial
          alpha = alpha_c(i,j,k)
          beta(:) = beta_c(:,i,j,k)
          g(:) = gamma_c(:,i,j,k)
          ...
          ! Fase 1: Ignoramos la atmósfera temporalmente (ponemos 0 puro)
          if (r_loc < r_in) then
            prim_state(eq_de:eq_vz, i, j, k) = 0.0d0
            cycle
          end if
    ...
    ! Fase 2: Encontrar el máximo real
    rho_max_local = maxval(prim_state(eq_de, :, :, :))
  
    ! Fase 3: Reescalar y blindar con el piso térmico globalmente
    !!$OMP PARALLEL DO PRIVATE(i, j, k)
    do k = 1, local_nz
      do j = 1, local_ny
        do i = 1, local_nx
          ! Solo reescalamos donde hay toroide (de > 0)
          if (prim_state(eq_de, i, j, k) > 0.0d0) then
            prim_state(eq_de, i, j, k) = prim_state(eq_de, i, j, k) / rho_max_local
            prim_state(eq_pr, i, j, k) = prim_state(eq_pr, i, j, k) / rho_max_local
          end if
          
          ! AHORA blindamos el universo: si cayó por debajo del piso (o era cero), es atmósfera
