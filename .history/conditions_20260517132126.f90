...
contains
  ...
  ! ========================================================================
  ! ========================================================================
  ! TORO DE FISHBONE-MONCRIEF (Física Universal ADM y Fronteras Seguras)
  ! ========================================================================
  subroutine setup_fishbone_moncrief_initial(prim_state, r_in, r_max_dens, K_poly)
    use variables ! Para acceder a x_min, y_min, dx, dy, etc.
    implicit none
    ...
    ! CÁLCULO DE W_in EN EL BORDE INTERNO
    if (use_log_r) then
      coord_in = log(r_in)
    else
      coord_in = r_in
    ...
    !!$OMP PARALLEL DO PRIVATE(i, j, k, coord_x, coord_y, r_loc, alpha, beta, g, ii, jj, g_tt, g_tphi, g_phiphi, ut_sq, Omega, W_r, enthalpy)
    do k = 1, local_nz
      do j = 1, local_ny
        do i = 1, local_nx
          
          ! 1. CÁLCULO SEGURO DE COORDENADAS (Soporta Celdas Fantasma sin leer memoria basura)
          coord_x = x_min + (dble(i) - 0.5d0) * dx 
          coord_y = y_min + (dble(j) - 0.5d0) * dy
          ...
          ! 2. LECTURA GEOMÉTRICA DIRECTA (Ignora la caché para no colapsar las fronteras)
          call calculate_metric(coord_x, coord_y, alpha=alpha, beta=beta, gamma=g)
