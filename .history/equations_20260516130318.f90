...
contains
  ...
  subroutine cons_to_prim(cons_state, prim_state, i, j, k)
    ...
    real*8 :: g(3,3), ginv(3,3), det_g, sqg
    real*8 :: S_cov(3), S_con(3)
    integer :: ii, jj
    ...
    ! --- INVERSIÓN ANALÍTICA DE LA MÉTRICA ESPACIAL (Regla de Cramer) ---
    ...
    ginv(2,3) = (g(1,2)*g(3,1) - g(1,1)*g(3,2)) / det_g
    ginv(3,1) = ginv(1,3)
    ginv(3,2) = ginv(2,3)
    ...
    if (S_sq > S_sq_max .and. S_sq > 1.0d-20) then
      ...
      S_con = S_con * scale_factor  ! Saneamiento vital para la extracción de velocidad
    end if
