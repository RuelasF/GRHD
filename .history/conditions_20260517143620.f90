...
contains
  ...
  subroutine setup_fishbone_moncrief_initial(prim_state, r_in, r_max_dens, K_poly)
    ...
    real*8  :: g_tt, g_tphi, g_phiphi, Omega, coord_in, denom

    local_nx = size(prim_state, 2)
    local_ny = size(prim_state, 3)
    local_nz = size(prim_state, 4)

    ! CÁLCULO DE W_in (Usando el puntero métrico general en el ecuador)
    if (use_log_r) then
      coord_in = log(r_in)
    else
      coord_in = r_in
    ...
    denom = g_phiphi + 2.0d0 * l_ang * g_tphi + l_ang**2 * g_tt
    W_in  = 0.5d0 * log(((g_tphi)**2 - g_tt * g_phiphi) / denom)

    !!$OMP PARALLEL DO PRIVATE(i, j, k, r_loc, alpha, beta, g, ii, jj, g_tt, g_tphi, g_phiphi, denom, ut_sq, Omega, W_r, enthalpy)
    do k = 1, local_nz
      do j = 1, local_ny
        do i = 1, local_nx
          ...
          ! Blindaje de frontera interna física
          if (r_loc < r_in) then
          ...
          ! --- PROTECCIÓN CLAVE: CONSISTENCIA GEOMÉTRICA ABSOLUTA ---
          if (i >= 1 .and. i <= nx .and. j >= 1 .and. j <= ny) then
            ! Forzamos concordancia exacta de bits con los arreglos de evolución
            alpha   = alpha_c(i,j,k)
            beta(:) = beta_c(:,i,j,k)
            g(:,:)  = gamma_c(:,:,i,j,k)
          else
            ! Celdas fantasma (Fuera de la caché): Evaluamos con la coordenada exacta de la malla
            call calculate_metric(x(i), y(j), alpha=alpha, beta=beta, gamma=g)
          end if
          ...
          if (denom > 1.0d-14) then
            ut_sq = ((g_tphi)**2 - g_tt * g_phiphi) / denom
          else
            ut_sq = -1.0d0
          end if
    ...
    print *, ">>> Toro FM Universal ADM balanceado. W_in =", W_in
