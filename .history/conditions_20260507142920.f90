...
contains
  ...
  subroutine setup_michel_accretion_initial(prim_state, r_critical, rho_critical, amp)
    ...
    real*8 :: W_lorentz, g(3), beta(3), alpha
    ...
    ! Corrección: Actualización de la bandera global
    if (use_log_r) then
      r_phys = exp(x) 
    else
      r_phys = x
    ...
    ! Constantes de la solución asintótica evaluadas en el radio crítico
    ...
    do i = 1, local_nx
      ...
      ! Inyección opcional de un pulso de densidad gaussiano
      rho_final = rho
      if (present(amp)) rho_final = rho + amp * exp(-0.5d0*((r_phys(i) - 30.0d0)/2.0d0)**2)

      ! Mapeo a la malla 3D/2D
      do k = 1, local_nz
        do j = 1, local_ny
          ...
          ut = (1.0d0 + g(1) * ux**2) / (ut_tmp - beta_c(1,i,j,k) * g(1) * ux)
          W_lorentz = alpha * ut
  ...
  ! ========================================================================
  ! TORO DE FISHBONE-MONCRIEF (Versión Unificada y Generalizada 3D)
  ! ========================================================================
  subroutine setup_fishbone_moncrief_initial(prim_state, r_in, r_max_dens, K_poly)
    ...
    integer :: local_nx, local_ny, local_nz
    integer :: i, j, k
    real*8  :: l_ang, W_in, W_r, enthalpy, ut_sq, r_loc, th_loc, sin_th
    real*8  :: alpha, beta(3), g(3)
    ...
    ! CORRECCIÓN 1: Adaptar el parámetro físico a la coordenada de la malla 
    ! para que calculate_metric lo procese correctamente.
    if (use_log_r) then
      coord_in = log(r_in)
    else
      coord_in = r_in
    ...
    ! El momento angular se define en el plano ecuatorial (pi/2)
    call calculate_metric(coord_in, pi/2, alpha=alpha, beta=beta, gamma=g)
    l_ang = sqrt(bh_mass * r_max_dens) / (1.0d0 - 2.0d0 * bh_mass / r_max_dens)
    g_tt = -alpha**2 + g(1)*beta(1)**2 + g(2)*beta(2)**2 + g(3)*beta(3)**2
    ut_sq = (-g_tt * g(3)) / (g(3) + l_ang**2 * g_tt)
    W_in  = 0.5d0 * log(ut_sq)

    !!$OMP PARALLEL DO PRIVATE(i, j, k, r_loc, th_loc, sin_th, g_tt, alpha, beta, g, ut_sq, W_r, enthalpy)
    do k = 1, local_nz
      do j = 1, local_ny
        do i = 1, local_nx
          
          ! CORRECCIÓN 2: Recuperar el radio físico real para las ecuaciones de estado 
          ! y los límites geométricos del Toro.
          if (use_log_r) then
            r_loc = exp(x(i))
          else
            r_loc = x(i)
          ...
          ! Protección numérica extrema usando el radio físico
          if (r_loc < r_in .or. sin_th < 1.0d-4) then
          ...
          ! Verificamos que el radicando no se indetermine
          if (g(3) + l_ang**2 * g_tt > 0.0d0) then
            ut_sq = (-g_tt * g(3)) / (g(3) + l_ang**2 * g_tt)
          else
            ut_sq = -1.0d0 
          end if
          ...
          if (ut_sq > 0.0d0) then
            ...
            if (enthalpy > 1.0d0) then
              ...
              prim_state(eq_vz, i, j, k) = ( l_ang * (-g_tt) / g(3) + beta(3) ) / alpha
    ...
    ! Normalización de la densidad para que rho_max = 1.0
  ! rho_max_local = maxval(prim_state(eq_de, :, :, :))
  
  !   do k = 1, nz
  !     do j = 1, ny
  !       do i = 1, nx
  !         if (prim_state(eq_de, i, j, k) > rho_floor) then
  !           ! Escalar densidad y presión (manteniendo entropía/K_poly constante)
  !           prim_state(eq_de, i, j, k) = prim_state(eq_de, i, j, k) / rho_max_local
  !           prim_state(eq_pr, i, j, k) = prim_state(eq_pr, i, j, k) / (rho_max_local**adb_idx)
  !         end if
  !       end do
  !     end do
  !   end do
