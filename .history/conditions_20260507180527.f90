...
contains
  ...
  subroutine setup_fishbone_moncrief_initial(prim_state, r_in, r_max_dens, K_poly)
    ...
    real*8  :: g_tt, coord_in, rho_max_local

    local_nx = size(prim_state, 2)
    local_ny = size(prim_state, 3)
    local_nz = size(prim_state, 4)

    ! RESTAURAMOS TU CORRECCIÓN 1:
    ...
    ! Fase 2: Encontrar el máximo real del disco
    rho_max_local = maxval(prim_state(eq_de, :, :, :))
  
    ! Fase 3: Reescalar el disco y luego aplicar el piso atmosférico globalmente
    !!$OMP PARALLEL DO PRIVATE(i, j, k)
    do k = 1, local_nz
      do j = 1, local_ny
        do i = 1, local_nx
          ! Solo reescalamos el gas real del toroide
          ...
          ! Blindaje final: rellenamos el vacío con la atmósfera ligera
