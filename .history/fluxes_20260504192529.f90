...
module fluxes
  ...
  ! ====================================================================
  abstract interface
    subroutine wavespeed_interface(prim_state, sweep_dir, alpha, beta, g, wave_min, wave_max)
      implicit none
      real*8, intent(in)  :: prim_state(:)
      integer, intent(in) :: sweep_dir
      real*8, intent(in)  :: alpha, beta(3), g(3)
...
contains
  ...
  subroutine prim_to_cons_face(prim_state, cons_state, g, sqg)
    implicit none
    real*8, intent(in)  :: prim_state(:), g(3), sqg
    real*8, intent(out) :: cons_state(:)
    
    real*8 :: v_sq, h, W, v1, v2, v3
  ...
  subroutine calc_hlle_fluxes(q_L, q_R, sweep_dir, alpha, beta, g, sqg, flux_out)
    ...
    ! 2. Transformamos variables primitivas a conservadas (Usando la rutina auxiliar nueva)
    call prim_to_cons_face(q_L, u_L, g, sqg)
    call prim_to_cons_face(q_R, u_R, g, sqg)

    ! 3. Cálculo de velocidades de onda
    call calc_wavespeeds(q_L, sweep_dir, alpha, beta, g, lambda_L_min, lambda_L_max)
    call calc_wavespeeds(q_R, sweep_dir, alpha, beta, g, lambda_R_min, lambda_R_max)
  ...
  subroutine calc_hllc_fluxes(q_L, q_R, sweep_dir, alpha, beta, g, sqg, flux_out)
    ...
    if (abs(A_quad) < 1.0d-14) then
      ...
      if (discriminant < 0.0d0) then
        flux_out = f_hll 
        ! Fallback silencioso a HLLE en lugar de un print para no saturar 
        ! la terminal en el barrido OMP
        return
      end if
  ...
  subroutine calc_wavespeeds_srhd(prim_state, sweep_dir, alpha, beta, g, wave_min, wave_max)
    implicit none
    real*8, intent(in)  :: prim_state(:)
    integer, intent(in) :: sweep_dir
    real*8, intent(in)  :: alpha, beta(3), g(3)
    ...
    real*8 :: v_norm, v1, v2, v3
    
    ! (alpha, beta, g no se usan explícitamente porque se asume espacio plano puro diag(1,1,1) y shift=0)
    ! Si se requiere un formalismo SRHD en coordenadas curadas (ej. esféricas), se usaría GRHD.

    v1 = prim_state(eq_vx)
    v2 = prim_state(eq_vy)
    v3 = prim_state(eq_vz)

    ! En Minkowski cartesiano la métrica es diag(1,1,1)
    v_sq = v1**2 + v2**2 + v3**2
  ...
  subroutine calc_wavespeeds_grhd(prim_state, sweep_dir, alpha, beta, g, wave_min, wave_max)
    implicit none
    real*8, intent(in)  :: prim_state(:)
    integer, intent(in) :: sweep_dir
    real*8, intent(in)  :: alpha, beta(3), g(3)
    ...
    ! Cuadrado de la velocidad con métrica espacial diagonal
    v_sq = v1**2 * g(1) + v2**2 * g(2) + v3**2 * g(3)
    ...
    if (sweep_dir == DIR_X) then
      v_norm = v1 ; beta_norm = beta(1) ; g_nn = g(1)
    else if (sweep_dir == DIR_Y) then
      v_norm = v2 ; beta_norm = beta(2) ; g_nn = g(2)
    else
      v_norm = v3 ; beta_norm = beta(3) ; g_nn = g(3)
    end if

    ! Ecuaciones acopladas con el lapso y el shift
    term1 = alpha / (1.0d0 - v_sq * cs_sq)
    term2 = cs * sqrt( (1.0d0 - v_sq) * (alpha / g_nn / term1 - v_norm**2 * (1.0d0 - cs_sq)) )
