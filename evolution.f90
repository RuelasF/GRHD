...
contains
  ...
  subroutine update_adaptive_dt()
    ...
    real*8 :: local_vx, local_vy, local_vz, alpha, beta(3), g_up(3)
    integer :: i, j, k
    ...
    !$OMP PARALLEL DO PRIVATE(i, j, k, alpha, beta, g_up, local_vx, local_vy, local_vz) &
    !$OMP REDUCTION(max:max_vx, max_vy, max_vz) COLLAPSE(3)
    do k = 1, nz
      do j = 1, ny
        do i = 1, nx
          
          ! Leer métrica en el centro de la celda
          alpha  = alpha_c(i,j,k)
          beta(:) = beta_c(:,i,j,k)
          g_up(:) = gamma_c(:,i,j,k) ! Asumiendo que g_up se refiere a g_ii espacial
          
          ! Límite Causal Absoluto Físico (Cono de Luz Coordenado: ds^2 = 0)
          ! La velocidad coordenada de la luz es c^i = alpha / sqrt(g_ii) + |beta^i|
          local_vx = alpha / sqrt(g_up(1)) + abs(beta(1))
          local_vy = alpha / sqrt(g_up(2)) + abs(beta(2))
          local_vz = alpha / sqrt(g_up(3)) + abs(beta(3))
